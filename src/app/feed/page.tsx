...
export default async function FeedPage({ searchParams }: { searchParams?: { limit?: string } }) {
  ...
  const [{ data: publicProfiles }, { data: countRows }, { data: viewerProfile }] = await Promise.all([
    supabase
      .from("profiles")
  ...
  return (
    <div className="mx-auto max-w-2xl px-5 py-10">
      ...
      {followeeIds.length === 0 ? (
        ...
        <>
          {showCaughtUpBanner && (
            <div className="mt-8 flex items-center gap-2 rounded-full border border-line bg-surface px-4 py-2.5 text-sm text-muted">
              <CheckCircle2 size={16} className="text-accent" aria-hidden />
              You&rsquo;re all caught up.
            </div>
          )}
          <ul className={cn("space-y-6", showCaughtUpBanner ? "mt-6" : "mt-8")}>
            {items.map((item) => {

...
export type SpotifyTrackChoice = { id: string; name: string; artist: string; image: string | null };
...
export function TrackPicker({
  ...
  value: SpotifyTrackChoice | null;
  onChange: (track: SpotifyTrackChoice | null) => void;
  placeholder?: string;
}) {
  const [query, setQuery] = useState("");
  const [results, setResults] = useState<SpotifyTrackChoice[]>([]);
  ...
  if (value) {
    return (
      <div className="flex items-center gap-3 rounded-lg border border-line bg-surface px-3 py-2">
        {value.image ? (
          <Image src={value.image} alt="" width={36} height={36} className="h-9 w-9 shrink-0 rounded object-cover" />
        ) : (
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded bg-raised text-muted">
            <Music2 size={16} />
          </span>
        )}
        <span className="min-w-0 flex-1">
          <span className="block truncate text-sm font-medium">{value.name}</span>
          {value.artist && <span className="block truncate text-xs text-muted">{value.artist}</span>}
        </span>

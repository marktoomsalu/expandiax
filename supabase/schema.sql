...
-- ---------- Comments ----------
-- Same kind/target_id polymorphic pattern as likes. Deletion is allowed for
-- the commenter (their own words) and the owner of the target content
-- (moderating what's said about their own trip/event).
...
-- Written exclusively by security-definer trigger functions attached to
-- likes/comments/follows — no insert policy for regular users, so the app
-- can only ever read and mark its own notifications as read.
...
-- @capacitor/push-notifications). Users manage their own tokens; only
-- the push-send API route (service-role client) reads them otherwise.
